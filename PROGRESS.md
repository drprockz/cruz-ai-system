...
| Task | Description | Status |
|---|---|---|
| 2.1 | `agents/forge/forge_agent.py` — shell exists, tests pass | ⚠️ Stub only — no real file I/O, no linter loop |
| 2.2 | `agents/echo/echo_agent.py` — approval gate implemented | ⚠️ Stub only — uses Claude not Qwen, no Gmail integration |
...
| 3.5 | `agents/pm/pm_agent.py` — Qwen sprint planning + Linear/JIRA integration | ❌ Not built |

**What's missing to close Phase 3:**
- `workers/` directory + ARQ worker entrypoint + 3 cron jobs
- REACH agent (2-stage: Gemini discovery → Qwen personalization)
- CATCH agent (calendar trigger → audio capture → Whisper → Llama → Notion → Linear)
- PM agent (sprint breakdown → Linear tasks → Monday 9AM auto-review)
